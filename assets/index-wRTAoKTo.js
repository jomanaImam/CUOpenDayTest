(function(){const r=document.createElement("link").relList;if(r&&r.supports&&r.supports("modulepreload"))return;for(const e of document.querySelectorAll('link[rel="modulepreload"]'))s(e);new MutationObserver(e=>{for(const o of e)if(o.type==="childList")for(const n of o.addedNodes)n.tagName==="LINK"&&n.rel==="modulepreload"&&s(n)}).observe(document,{childList:!0,subtree:!0});function a(e){const o={};return e.integrity&&(o.integrity=e.integrity),e.referrerPolicy&&(o.referrerPolicy=e.referrerPolicy),e.crossOrigin==="use-credentials"?o.credentials="include":e.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function s(e){if(e.ep)return;e.ep=!0;const o=a(e);fetch(e.href,o)}})();function M(i,r,a,s,e,o,n){const t=`https://placehold.co/600x400/D50032/ffffff?text=${encodeURIComponent(r)}`,u=n||t,l=`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(`${s} ${o} Cardiff University UK`)}`,c=e||o?`<a href="${l}" target="_blank" rel="noopener noreferrer" onclick="event.stopPropagation()" class="text-xs text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 font-semibold hover:underline mt-1 pl-6 block relative z-20 transition-colors cursor-pointer w-fit">
            ${e} ${o}
           </a>`:"";return`
        <div data-topic-id="${i}" class="event-card relative group cursor-pointer transition-all duration-300 h-full" onpointerdown="this.startY = event.clientY" onpointerup="if(window.innerWidth < 768 && Math.abs(event.clientY - this.startY) < 10) this.classList.toggle('is-tapped')">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex justify-between items-center h-16">
        ...
        <div id="logo-home-btn" class="flex items-center gap-3 overflow-hidden cursor-pointer hover:opacity-80 transition-opacity" title="Return Home">
          <img src="https://jomanaimam.github.io/CUOpenDayTest/cu-logo.svg" alt="Cardiff University Logo" class="h-12 w-auto" />
  ...
  `;const s=a.querySelector("#desktop-search"),e=a.querySelector("#mobile-search-input"),o=a.querySelector("#mobile-search-container"),n=a.querySelector("#theme-toggle"),t=a.querySelector("#theme-toggle-dark-icon"),u=a.querySelector("#theme-toggle-light-icon"),d=window.matchMedia("(prefers-color-scheme: dark)"),l=m=>{m?(document.documentElement.classList.add("dark"),u?.classList.remove("hidden"),t?.classList.add("hidden")):(document.documentElement.classList.remove("dark"),u?.classList.add("hidden"),t?.classList.remove("hidden"))};localStorage.getItem("color-theme")==="dark"||!("color-theme"in localStorage)&&d.matches?l(!0):l(!1),d.addEventListener("change",m=>{"color-theme"in localStorage||l(m.matches)}),n?.addEventListener("click",()=>{const m=document.documentElement.classList.contains("dark");l(!m),m?localStorage.setItem("color-theme","light"):localStorage.setItem("color-theme","dark")});const c=m=>{const f=m.target.value;m.target===s&&(e.value=f),m.target===e&&(s.value=f),i(f)};s?.addEventListener("input",c),e?.addEventListener("input",c);const h=m=>{if(m.key==="Enter"){const f=m.target;f.blur(),f===e&&o?.classList.add("hidden")}};s?.addEventListener("keydown",h),e?.addEventListener("keydown",h);const g=()=>{s.value="",e.value="",i("")};return a.querySelector("#logo-home-btn")?.addEventListener("click",g),a.querySelector("#desktop-home-btn")?.addEventListener("click",g),a.querySelector("#mobile-search-toggle")?.addEventListener("click",()=>{o?.classList.toggle("hidden"),o?.classList.contains("hidden")||e?.focus()}),a.querySelector("#mobile-menu-btn")?.addEventListener("click",r),a}const C={"Physical Sciences & Engineering":["Architecture","Chemistry","Computer Science and Informatics","Earth and Environmental Sciences","Engineering","Mathematics","Physics and Astronomy"],"Biomedical & Life Sciences":["Biosciences","Dentistry and Dental Therapy and Hygiene","Healthcare Sciences","Medicine","Optometry and Vision Sciences","Pharmacy and Pharmaceutical Sciences (including Medical Pharmacology)","Psychology"],"Arts, Humanities & Social":["Business","English, Communication and Philosophy","Geography and Planning","History, Archaeology and Heritage","Journalism, Media and Culture","Law","Modern Languages","Music","Politics and International Relations","Social Sciences","Welsh"],"Campus Life":["Sport","Students' Union","General Talks and Activities","Residences"]};function I(i){const r=document.createElement("div");r.className="w-full bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 shadow-sm relative z-40";let a=`
    ...
    `;Object.entries(C).forEach(([d,l])=>{a+=`
        ...
        `}),a+="</div>",s+="</div></div>";const e='<div id="sidebar-overlay" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-40 hidden sm:hidden transition-opacity"></div>';r.innerHTML=a+s+e,r.querySelectorAll(".dept-btn").forEach(d=>{d.addEventListener("click",l=>{const c=l.currentTarget.getAttribute("data-dept")||"";i(c),u()})}),r.querySelectorAll(".college-btn").forEach(d=>{d.addEventListener("click",l=>{const c=l.currentTarget.getAttribute("data-college")||"";i(c),u()})}),r.querySelectorAll(".mobile-accordion-btn").forEach(d=>{d.addEventListener("click",l=>{const c=l.currentTarget.nextElementSibling,h=l.currentTarget.querySelector("svg");c?.classList.toggle("hidden"),h?.classList.toggle("rotate-180")})});const o=r.querySelector("#mobile-sidebar"),n=r.querySelector("#sidebar-overlay"),t=r.querySelector("#close-sidebar-btn"),u=()=>{o?.classList.add("-translate-x-full"),n?.classList.add("hidden")};return t?.addEventListener("click",u),n?.addEventListener("click",u),r.toggleSidebar=()=>{o?.classList.remove("-translate-x-full"),n?.classList.remove("hidden")},r}function D(i,r){if(!i)return"Date TBC";try{const a=new Date(i),s=a.toLocaleDateString("en-GB",{day:"numeric",month:"short",year:"numeric"}),e={hour:"2-digit",minute:"2-digit",hour12:!0},o=a.toLocaleTimeString("en-GB",e);if(!r)return`${s} | ${o}`;const n=new Date(r).toLocaleTimeString("en-GB",e);return`${s} | ${o} - ${n}`}catch{return"Date & Time TBC"}}function q(i,r,a){const s=document.createElement("div");s.className="w-full rounded-2xl overflow-hidden shadow-2xl mb-8 relative group cursor-pointer bg-gray-900";const e=i.filter(n=>n.description);e.length===0&&e.push({description:"Welcome to Cardiff Open Day",cover_image:null});let o='<div id="carousel-track" class="flex transition-transform duration-700 ease-in-out h-[350px] sm:h-80 md:h-96 w-full">';if(e.forEach(n=>{const t=n.cover_image||"https://cardiff.imgix.net/__data/assets/image/0012/657669/Open-Day-banner-image-small.jpg?w=1000&fit=crop&q=60&auto=format",u=n.description||"Cardiff University Event",d=D(n.start_time,n.end_time),l=n.id&&r.includes(String(n.id)),c=!!n.website,h=l||c,g=h?"line-clamp-3 md:line-clamp-2":"line-clamp-5 md:line-clamp-4",m=h?`
        ...
        `:"";o+=`
            <div class="w-full min-w-full max-w-full relative h-full flex-shrink-0 overflow-hidden">
                <img src="${t}" class="w-full h-full object-cover opacity-60">
        ...
        `}),o+="</div>",e.length>1&&(o+=`
            ...
            <div class="hidden md:flex absolute bottom-6 right-6 space-x-2 z-20 max-w-[50%] flex-wrap justify-end gap-y-2">
                ${e.map((n,t)=>`<button class="dot-btn w-2.5 h-2.5 rounded-full bg-white/40 hover:bg-white transition-all shadow-sm flex-shrink-0 m-1" data-index="${t}"></button>`).join("")}
            </div>
        `),s.innerHTML=o,s.querySelectorAll(".hero-read-more").forEach(n=>{n.addEventListener("click",t=>{t.stopPropagation();const u=t.currentTarget.getAttribute("data-id"),d=t.currentTarget.getAttribute("data-website");u&&a?a(u):d&&window.open(d,"_blank")})}),e.length>1){const n=s.querySelector("#carousel-track"),t=s.querySelectorAll(".dot-btn"),u=s.querySelector("#prev-btn"),d=s.querySelector("#next-btn");let l=0,c=null,h=!1;const g=b=>{l=b,l<0&&(l=e.length-1),l>=e.length&&(l=0),n.style.transform=`translateX(-${l*100}%)`,t.forEach((w,E)=>{E===l?(w.classList.replace("bg-white/40","bg-white"),w.classList.add("scale-150")):(w.classList.replace("bg-white","bg-white/40"),w.classList.remove("scale-150"))})},m=()=>g(l+1),f=()=>g(l-1),y=()=>{!c&&!h&&(c=setInterval(m,3500))},x=()=>{c&&(clearInterval(c),c=null)};s.addEventListener("mouseenter",()=>{h=!0,x()}),s.addEventListener("mouseleave",()=>{h=!1,y()}),s.addEventListener("click",()=>{h=!0,x(),setTimeout(()=>{h=!1,y()},5e3)}),d?.addEventListener("click",b=>{b.stopPropagation(),m(),x()}),u?.addEventListener("click",b=>{b.stopPropagation(),f(),x()}),t.forEach((b,w)=>{b.addEventListener("click",E=>{E.stopPropagation(),g(w),x()})}),g(0),y()}return s}function S(i,r){if(!i)return"Check Schedule";try{const a={hour:"2-digit",minute:"2-digit",hour12:!0},s=new Date(i).toLocaleTimeString("en-GB",a);if(!r)return s;const e=new Date(r).toLocaleTimeString("en-GB",a);return`${s} - ${e}`}catch{return"Various Times"}}function A(i){if(!i)return"Date TBC";try{return new Date(i).toLocaleDateString("en-GB",{weekday:"short",month:"short",day:"numeric"})}catch{return"Date TBC"}}let v=[],p=null,B=[];async function _(){try{const[i,r]=await Promise.all([fetch("api/OpenDay.json"),fetch("api/OpenDayList.json").catch(()=>null)]);p=await i.json(),v=p.topics||[],B=r&&r.ok?await r.json():[p],H(),k(v)}catch(i){console.error("Failed to load data:",i)}}function $(i){const r=document.getElementById("hero-container"),a=document.getElementById("main-grid-view"),s=document.getElementById("detail-view-container");if(a?.classList.remove("hidden"),s?.classList.add("hidden"),!i||i.trim()===""){L("All Departments & Events"),k(v),r?.classList.remove("hidden"),window.scrollTo({top:0,behavior:"smooth"});return}r?.classList.add("hidden");const e=v.filter(t=>t.name&&t.name.toLowerCase()===i.toLowerCase());if(e.length>0){L(e[0].name),k(e),window.scrollTo({top:0,behavior:"smooth"});return}if(C[i]){const t=C[i],u=v.filter(d=>d.name&&t.includes(d.name));L(`Departments in: ${i}`),k(u),window.scrollTo({top:0,behavior:"smooth"});return}const o=i.toLowerCase().replace(/,/g," ").split(/\s+/).filter(t=>t.length>0),n=v.filter(t=>{const u=(t.name||"").toLowerCase();let d="",l="",c="",h="";if(t.programs&&t.programs.length>0&&t.programs[0].location){const m=t.programs[0].location;d=(m.title||m.building_name||"").toLowerCase(),l=(m.address||"").toLowerCase(),c=(m.postcode||"").toLowerCase(),h=c.replace(/\s+/g,"")}const g=`${u} ${d} ${l} ${c} ${h}`;return o.every(m=>g.includes(m))});L(`Search Results for "${i}"`),k(n),window.scrollTo({top:0,behavior:"smooth"})}function L(i){const r=document.getElementById("grid-title");r&&(r.innerText=i)}function H(){const i=I(n=>$(n)),r=j($,()=>i.toggleSidebar());document.body.insertBefore(i,document.body.firstChild),document.body.insertBefore(r,i);const a=document.getElementById("app");if(!a)return;a.innerHTML=`
    ...
    `;const s=v.map(n=>String(n.id)),e=q(B,s,n=>T(n));document.getElementById("hero-container")?.appendChild(e);const o=document.getElementById("back-to-top-btn");window.addEventListener("scroll",()=>{window.scrollY>400?(o?.classList.remove("translate-y-16","opacity-0","pointer-events-none"),o?.classList.add("translate-y-0","opacity-100","pointer-events-auto")):(o?.classList.add("translate-y-16","opacity-0","pointer-events-none"),o?.classList.remove("translate-y-0","opacity-100","pointer-events-auto"))},{passive:!0}),o?.addEventListener("click",()=>{window.scrollTo({top:0,behavior:"smooth"})})}function k(i){const r=document.getElementById("gridContainer");if(!r)return;if(i.length===0){r.innerHTML='<div class="col-span-full text-center text-gray-500 py-12 bg-white rounded-xl border border-dashed border-gray-300">No events found matching your search.</div>';return}let a="",s="Check Schedule";p&&p.start_time&&p.end_time&&(s=S(p.start_time,p.end_time)),i.forEach(e=>{const o=e.name||"Cardiff Event";let n="Cardiff University Campus",t="",u="";if(e.programs&&e.programs.length>0){const l=e.programs[0];l.location&&(n=l.location.title||l.location.building_name||"Cardiff University Campus",t=l.location.address||"",u=l.location.postcode||"")}const d=e.cover_image||p&&p.cover_image||"";a+=M(e.id,o,s,n,t,u,d)}),r.innerHTML=a,document.querySelectorAll(".event-card").forEach(e=>{e.addEventListener("click",()=>{const o=e.getAttribute("data-topic-id");T(o)})})}function T(i){if(!i)return;const r=v.find(t=>String(t.id)===String(i));if(!r)return;const a=document.getElementById("detail-view-container");if(!a)return;const s=r.cover_image||p&&p.cover_image||`https://placehold.co/1200x400/e11d48/ffffff?text=${encodeURIComponent(r.name)}`,e=r.website||`https://www.cardiff.ac.uk/search?query=${encodeURIComponent(r.name)}`;let o=`
        ...
        <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-800 overflow-hidden mb-10">
            ...
            <div class="p-6 sm:p-10">
                ...
                <a href="${e}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center bg-red-700 hover:bg-red-800 text-white font-bold py-3 px-6 rounded-xl transition-colors shadow-sm">
    ...
    `;const n=r.programs||[];n.length>0?(o+='<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 items-stretch">',n.forEach(t=>{const u=t.title||t.name||"General Event",d=S(t.start_time,t.end_time),l=A(t.start_time||p?.start_time),c=t.description||"Join us for this scheduled session to explore our facilities, meet the staff, and learn more about this specific program.";let h=t.location?.title||t.location?.building_name||"Location TBC",g=t.room||t.room_name||t.location_name||"",m=h;g&&g!==h&&g!=="TBC"&&(m=`${h} <br/> <span class="text-xs text-gray-500 dark:text-gray-400 font-normal leading-tight block mt-0.5">${g}</span>`);let f='<div class="mt-auto opacity-70 text-sm italic border-t border-red-500 pt-3">Location TBC</div>';if(t.location?.address||t.location?.postcode){const y=`${h} ${t.location.address||""} ${t.location.postcode||""} Cardiff UK`.replace(/\s+/g," ").trim();f=`
            ...
            `}),o+="</div>"):o+='<div class="bg-white dark:bg-gray-800 p-8 rounded-xl border border-gray-200 dark:border-gray-700 text-center text-gray-500 italic shadow-sm">No specific scheduled events found. Visit their main booth anytime!</div>',a.innerHTML=o,document.querySelectorAll(".flip-card-container").forEach(t=>{t.addEventListener("click",u=>{if(u.target.closest("a"))return;const d=t.querySelector(".flip-inner"),l=t.querySelector(".flip-glow"),c=d?.classList.contains("[transform:rotateY(180deg)]");document.querySelectorAll(".flip-inner").forEach(h=>{h.classList.remove("[transform:rotateY(180deg)]")}),document.querySelectorAll(".flip-glow").forEach(h=>{h.classList.remove("opacity-60")}),c||(d?.classList.add("[transform:rotateY(180deg)]"),l?.classList.add("opacity-60"))})}),document.getElementById("hero-container")?.classList.add("hidden"),document.getElementById("main-grid-view")?.classList.add("hidden"),a.classList.remove("hidden"),document.getElementById("backToGridBtn")?.addEventListener("click",()=>{a.classList.add("hidden"),document.getElementById("main-grid-view")?.classList.remove("hidden");const t=document.getElementById("desktop-search");(!t||t.value.trim()==="")&&document.getElementById("hero-container")?.classList.remove("hidden"),window.scrollTo({top:0,behavior:"smooth"})}),window.scrollTo({top:0,behavior:"smooth"})}_();
